rs-// SPDX-License-Identifier: MIT LICENSE
...
contract Traits is Ownable, ITraits {
  ...
  function drawSVG(uint256 tokenId) public view returns (string memory) {
    ...
    uint8 shift = s.isVirgin ? 0 : 9;

    string memory svgString = string(abi.encodePacked(
      drawTrait(traitData[0 + shift][s.fur]),
      s.isVirgin ? drawTrait(traitData[1 + shift][s.head]) : drawTrait(traitData[1 + shift][s.alphaIndex]),
      s.isVirgin ? drawTrait(traitData[2 + shift][s.ears]) : '',
      drawTrait(traitData[3 + shift][s.eyes]),
      s.isVirgin ? drawTrait(traitData[4 + shift][s.nose]) : '',
      drawTrait(traitData[5 + shift][s.mouth]),
      s.isVirgin ? '' : drawTrait(traitData[6 + shift][s.neck]),
      s.isVirgin ? drawTrait(traitData[7 + shift][s.feet]) : ''
    ));
